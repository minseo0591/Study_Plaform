...
public class comuSearchController implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("comuSearchController안의 execute() 실행됨");
		
		//1. 한글처리
		request.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		//model 객체 생성
		comuSearchDAO csd = new comuSearchDAO();
		...
		int listSearchCount = csd.getComuSearchCount(keyWord, searchWord);
		comulist = csd.comulistSearch(page, limit, keyWord, searchWord);
