...
public class MemberListController implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("MemberListController안의 execute() 실행됨");
		//1. 한글처리
		request.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		
		MemberDAO mbdao = new MemberDAO();
		ArrayList<Member> mbdto = new ArrayList<>();
		...
		int listcount = mbdao.getListCount(); 
		mbdto = mbdao.memberlist(page,limit); 	// 게시물을 LIST 객체에 담습니다. 
		...
		request.setAttribute("memberlist", mbdto);
		System.out.println("MemberListController : 디비작업처리완료");
