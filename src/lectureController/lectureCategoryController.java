...
public class lectureCategoryController implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int ClasslistCategoryCount = lcdao.ClasslistCategoryCount(category);
		lcdto = lcdao.ClasslistCategory(page, limit, category); 	// 게시물을 LIST 객체에 담습니다. 
		
		
		// 최대 페이지를 구합니다.
   		int maxpage = (int)((double)ClasslistCategoryCount / limit + 0.95); // 0.95 올림처리
		...
		request.setAttribute("count",ClasslistCategoryCount); 	// 게시물 총 개수
		...
		System.out.println("count :" + ClasslistCategoryCount);
		System.out.println("page :" + page);
		...
		System.out.println("lectureSearchController : 디비작업처리완료");
		
		//4. 페이지이동 : 무조건 Controller로 이동
		System.out.println("lectureCategorySearchController : 페이지이동");
